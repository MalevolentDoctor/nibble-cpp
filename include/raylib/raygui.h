...
int GuiValueBox(Rectangle bounds, const char *text, int *value, int minValue, int maxValue, bool editMode)
{
    ...
    sprintf_s(textValue, RAYGUI_VALUEBOX_MAX_CHARS + 1, "%i", *value);
            ...
            while (!feof(rgsFile))
            {
                switch (buffer[0])
                {
                    ...
                    case 'f':
                    {
                        ...
                        sscanf_s(buffer, "f %d %s %s", &fontSize, charmapFileName, (unsigned int)_countof(charmapFileName), fontFileName, (unsigned int)_countof(fontFileName));
...
const char *GuiIconText(int iconId, const char *text)
{
    ...
    if (text != NULL)
    {
        ...
        sprintf_s(buffer, 1024, "#%03i#", iconId);
    ...
    {
        sprintf_s(iconBuffer, 6, "#%03i#", iconId & 0x1ff);
